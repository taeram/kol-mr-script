...
function ResultHandler(e) {
    if (e.originalEvent.animationName == 'nodeInserted') {
        ...
        let mystuff = $(target).parents('#effdiv').children('center:first').html();
        let effdiv = $(target).parents('#effdiv');
// TODO:
// we need to check multiple <b> nodes here, for cases where there is, for example, multi-stage crafting.
        let bnode = effdiv.find('b:eq(1)');
        let btext = bnode.text();
        let goHere = checkForRedirects(mystuff);
        if (goHere != "") {
            ...
            let theItem = $(bnode).parent().parent().get(0);
            ...
            let insertAt = effdiv.find('blockquote:first');
            if (insertAt.length === 0)
                insertAt = effdiv.children('center:first').find('tr:last')
...
function process_effect(effectname, jnode) {
    switch(effectname) {
        case 'Filthworm Larva Stench':        jnode.after(Util.createLink('[drone chamber (1)]',Util.getSnarfblatUrl(128)));             break;
        case 'Filthworm Drone Stench':        jnode.after(Util.createLink('[guard chamber (1)]',Util.getSnarfblatUrl(129)));             break;
        case 'Filthworm Guard Stench':        jnode.after(Util.createLink('[Queen! (1)]',Util.getSnarfblatUrl(130)));                    break;
        case 'Stone-Faced':                    jnode.after(Util.createLink('[hidden temple (1)]',Util.getSnarfblatUrl(280)));            break;
        case 'Down the Rabbit Hole':        jnode.after(Util.createLink('[go down]',Util.getPlaceUrl('rabbithole')))
                                            .after(Util.createLink('[tea party!]',Util.getPlaceUrl('rabbithole&action=rabbithole_teaparty')));    break;
        case 'Knob Goblin Perfume':            jnode.after(Util.createLink('[knob]','cobbsknob.php'));                        break;
    }
}
...
function process_equip(itemname, jnode) {
    switch(itemname) {
        case 'continuum transfunctioner':    jnode.after(Util.createLink('[8-bit realm (1)]',Util.getSnarfblatUrl(73)));        break;
        case 'huge mirror shard':            jnode.after(Util.createLink('[chamber]','lair6.php?place=1'));        break;
        case 'snorkel':                        jnode.after(Util.createLink('[map]',inv_use(26)));                    break;
        case 'pool cue':                    jnode.after(Util.createLink('[chalk it!]',inv_use(1794)));            break;
        case "Talisman o' Nam":                jnode.after(Util.createLink('[Dome moD]',Util.getPlaceUrl('palindome')));    break;
        case 'worm-riding hooks':            jnode.after(Util.createLink('[drum!]',inv_use(2328)));                break;
        case 'Mega Gem':                    jnode.after(Util.createLink("[Dr. Awkward Office (1)]",Util.getPlaceUrl('palindome&action=pal_droffice'))); break;
        case 'dingy planks':                jnode.after(Util.createLink('[boat]', inv_use(146)));                break;
        case "makeshift SCUBA gear":         jnode.after(Util.createLink('[odor]', 'lair2.php?action=odor'));        break;
        case 'worthless gew-gaw':
        case 'worthless knick-knack':
        case 'worthless trinket':           jnode.after(Util.createLink('[hermit]','hermit.php'));               break;
        case 'antique machete':             jnode.after(Util.createLink('[hidden city]','hiddencity.php'));      break;
        ...
        case "swashbuckling pants":
            if (weCameFrom("choice.php")) {
                jnode.after(Util.createLink('[visit Caronch (1)]',Util.getSnarfblatUrl(157)));
            }
            break;
        case "Victor, the Insult Comic Hellhound Puppet":
        case "observational glasses":
        case "hilarious comedy prop":        jnode.after(Util.createLink('[visit Mourn]','pandamonium.php?action=mourn'));     break;
    }
}
...
function process_outfit(outfitname, jnode) {
    switch(outfitname) {
        case 'Knob Goblin Harem Girl Disguise':
            jnode
                .after(Util.createLink('[perfume]',inv_use(307)))
                .after(Util.createLink('[knob]','cobbsknob.php'));
            break;
        case 'Knob Goblin Elite Guard Uniform':
            jnode.after(Util.createLink('[knob]','cobbsknob.php'));
            break;
        case 'Swashbuckling Getup':
            jnode.after(Util.createLink('[island]','island.php'));
            break;
        case 'Filthy Hippy Disguise':
            if (weCameFrom('store.php')) {
                mainpane_goto('/store.php?whichstore=h');
            } else  {
                jnode.after(Util.createLink('[buy fruit]','store.php?whichstore=h'));
            }
            break;
        case 'Mining Gear':
            jnode.after(Util.createLink('[dwarf mine]','mining.php?mine=1'));
            break;
        case 'Bugbear Costume':
            if (weCameFrom('store.php')) {
                mainpane_goto('/store.php?whichstore=b');
            } else {
                jnode.after(Util.createLink('[bakery]','store.php?whichstore=b'));
            }
            break;
        case 'eXtreme Cold-Weather Gear':
            jnode.after(Util.createLink('[Trapper]','trapper.php'));
            jnode.after(Util.createLink('[hit the slopes (1)]',Util.getSnarfblatUrl(273)));
            break;
        case 'Cloaca-Cola Uniform':
        case 'Dyspepsi-Cola Uniform':
            jnode.after(Util.createLink('[battlefield (1)]',Util.getSnarfblatUrl(85)));
            break;
...
function equip(o) {
    let ie = "inv_equip.php?pwd=" + pwd + "&which=2";
...
function SetCharData(which, value) {
    let charname = GetData("charname");
    GM_setValue(charname + which, value);
}
function Character.get(which) {
    let charname = GetData("charname");
...
function FindMaxQuantity(item, howMany, deefault, safeLevel) {
    ...
    switch(integer(item))
    {
        ...
        case 518:                                 // Magical Mystery Juice
            min = 4 + (1.5 * Character.get("level")); max = min + 2; break;
    ...
    if (hp == 1) result = integer(Character.get("maxHP")-Character.get("currentHP"));
    else         result = integer(Character.get("maxMP")-Character.get("currentMP"));
...
function Util.createLink(linkString, linkURL,jaxify) {
...
function AppendBuyBox(itemNumber, whichStore, buttonText, noQuantityBox) {
    let eventString = ""; let htmlString = ""; let quantityString;
...
function NumberLink(b) {
    let num = b.textContent.split(' ')[0];
    while(num.indexOf(',') != -1) num = num.split(',')[0] + num.split(',')[1];
    num = integer(num);
    if (num < 26) {
        let txt = b.textContent.substring(
            b.textContent.indexOf(' '),b.textContent.length);
        let func = "var q = document.getElementsByName(\"quantity\");" +
...
function AppendOutfitSwap(outfitNumber, text) {
    ...
    // Revert to backup
    if (outfitNumber == 0) {
        GM_get( "account_manageoutfits.php", function(response) {
            let swap = $('input[name="swap"]');
            let val; let index2; let backup = GetPref('backup');
            let index = response.indexOf(' value="' + backup + '"');
...
function InvChecker(event) {
    ...
    //do I even need all this item stuff anymore??
    let item = this.parentNode.previousSibling.firstChild;
    if (item === null) {
        item = this.parentNode.previousSibling.firstChild.nextSibling;
    }
    if (item) item = item.getAttribute("value");
    if (item && item.length > 4) item = item.substring(0,item.length-9); // correct for prices in the mall
    //end do I
    let add = document.createElement('span');
    let br = document.createElement('br');
    let itemname = $(this).parent().next().text();

    $(add).attr('class','tiny').attr('id','span'+item).text("checking: "+itemname+ "...");
    $(this).parent().next().append(br).append(add);

    GM_get( 'submitnewchat.php?pwd='+pwd+'&graf=/count '+itemname, function(details) {
...
function ajaxit(dourl) {
    $.ajax({type:'GET',url:dourl+"&ajax=1", cache: false, data: null, global: false,
        success: function(out) {
            ...
            let $eff = $(top.mainpane.document).find("#effdiv");
            if ($eff.length == 0) {
                let d = top.mainpane.document.createElement('div');
                d.id = 'effdiv';
                let b = top.mainpane.document.body;
...
function AddLinks(descId, theItem, formWhere, path) {
    let itemNo = $(theItem).parents('.item');
    let itemQty = 0;
    let itemNum = 0;
    let rel = '';
    if (itemNo) {
    ...
    let doWhat, addWhere = $(theItem).children().eq(1);
    ...
    switch (integer(itemNum)) {
        ...
        case 2334: case    2766: case  2829:                                        // MacGuffin, solid gold bowling ball, really dense meat stack
            addWhere.append(Util.createLink('[council]','council.php',false)); break;
        ...
        case 4504: case 3291:                                                    // heart of volcano, secret tropical map
            addWhere.append(Util.createLink('[take to guild]','guild.php?place=scg')); break;
        case 2550: case 2551: case 2552: case 2553: case 2554: case 2555:        // LEW parts
            addWhere.append(Util.createLink('[smith]','craft.php?mode=smith')); break;
        case 454:                                                                 // rusty screwdriver
            addWhere.append(Util.createLink('[untinker]',Util.getPlaceUrl('forestvillage&action=fv_untinker'))); break;

        case 134:                                                                 // bitchin' meatcar
            addWhere.append(Util.createLink('[guild]','guild.php?place=paco')); break;

        case  459: case  460: case  461: case  462: case  463:                    // pixels
            addWhere.append(Util.createLink('[mystic]',Util.getPlaceUrl('forestvillage&action=fv_mystic'))); break;

        case  535:                                                                 // bridge
            addWhere.append(Util.createLink('[chasm]',Util.getPlaceUrl('orc_chasm&action=bridge0'))); break;

        case  602:                                                                 // Degrassi Knoll shopping list
            if (Character.get("friendlyknoll") == true) addWhere.append(Util.createLink('[gnoll store]', "store.php?whichstore=4"));
            else addWhere.append(Util.createLink('[Knoll]', Util.getPlaceUrl('knoll_hostile')));
            break;

        case  609:                                                              // S.O.C.K
            addWhere.append(Util.createLink('[giant Basement (1)]',Util.getSnarfblatUrl(322))); break;

        case  727:    case 728:                                                     // Hedge maze puzzle piece/key
            addWhere.append(Util.createLink('[maze]', 'hedgepuzzle.php', false)); break;

        case 1766:                                                              // ballroom key
            addWhere.append(Util.createLink('[ballroom (1)]',Util.getSnarfblatUrl(109))); break;
...
//            addWhere.append(Util.createLink('[use map]',Util.getInvUseUrl2054))); break;
        ...
        case 2182: case 2183:                                                     // Harold's hammer parts
            addWhere.append(Util.createLink('[make hammer]', 'craft.php?mode=combine' +
                                 '&action=craft&a=2182&b=2183&pwd=' + pwd +
                                 '&quantity=1')); break;
        case 2184:                                                                 // harold's hammer
            addWhere.append(Util.createLink('[give to Harold (1)]', Util.getSnarfblatUrl(112))); break;
        ...
        case 2277:                                                                 // Fernswarthy's key
            ...
            else addWhere.append(Util.createLink('[go to ruins]','fernruin.php'));
            break;

        case 2279:                                                                 // Dusty Old Book
            addWhere.append(Util.createLink('[take back to guild]','guild.php?place=ocg')); break;

        case 2326:                                                                // stone rose
            addWhere.append(Util.createLink('[Gnasir]',Util.getPlaceUrl('desertbeach&action=db_gnasir'))); break;

        case 3000:                                                                 // Caronch's dentures
            addWhere.append(Util.createLink("[equip swashbuckling pants]", equip({i:402}))); break;

        case 4668:                                                                 // observational glasses
            addWhere.append(Util.createLink("[visit Mourn]","pandamonium.php?action=mourn")); break;
        ...
        case   43: case  44: case  45:                                          // worthless thingies
            if (weCameFrom('hermit') && path == "/store.php") {
                ...
                addWhere.append(Util.createLink('[hermit]', 'hermit.php'));
            }
            break;

        case 1003:                                                                 // soda water
            addWhere
                .append(Util.createLink('[mix]', 'craft.php?mode=cocktail'))
                .append(Util.createLink('[still]', 'guild.php?place=still'));
            break;
        ...
        case   40:                                                                 // casino pass
            if (weCameFrom('casino') && path == "/store.php") {
                ...
                addWhere.append(Util.createLink('[casino]', 'casino.php'));
            }
            break;
        ...
        case  530:                                                                 // spray paint
            addWhere.append(Util.createLink('[the wall]', 'town_grafwall.php')); break;

        case   24:                                                                 // Clover
            addWhere.append(Util.createLink('[disassemble]', 'multiuse.php?pwd='+ pwd +
        ...
        case   47:                                                                 // Roll
            addWhere
            .append(Util.createLink('[casino]', 'casino.php'))
            .append(Util.createLink('[rock+roll]', 'craft.php?mode=smith&' +
                    'action=craft&a=47&b=30&pwd='+ pwd + '&quantity=1'));
            break;

        case   52:                                                                 // banjo Strings
            addWhere.append(Util.createLink('[twang]', 'craft.php?mode=smith&' +
                            'action=craft&a=52&b=30&pwd='+ pwd +
                            '&quantity=1'));
            break;

        case  135: case  136:                                                     // Rims, Tires
            addWhere.append(Util.createLink('[wheels]','craft.php?mode=combine&' +
                            'action=craft&a=135&b=136&pwd='+ pwd +
                            '&quantity=1'));
            break;

        case 2044:                                                                 // MacGuffin
            addWhere.append(Util.createLink('[read]',"diary.php?textversion=1")); break;

        case  485:                                                                 // snakehead charrrm: make talisman
            doWhat = 'oneuse';
//            addWhere.append(Util.createLink('[man, o nam]', 'craft.php?mode=combine&' +
//                            'action=craft&a=485&b=485&pwd='+ pwd +
//                            '&quantity=1'));
            break;

        case 2338:                                                                 // Black Pudding
            addWhere.append(Util.createLink('[eat]','inv_eat.php?pwd='+ pwd +
                            '&which=1&whichitem='+itemNum)); break;
        case 2054:                                                              // black market map
            //add link to switch to blackbird or crow
            addWhere.append(Util.createLink('[switch to blackbird]','familiar.php?action=newfam&newfam=59'));
                break;

        case 2064:                                                                 // Forged documents
            addWhere.append(Util.createLink('[shore]',Util.getPlaceUrl('desertbeach'))); break;
        case 2266:                                                                // wet stunt nut stew
            addWhere.append(Util.createLink('[visit Mr. Alarm]',Util.getPlaceUrl('palindome&action=pal_mroffice'))); break;
        case 2347:                                                                //heart of the filthworm queen
            addWhere.append(Util.createLink('[turn it in!]','bigisland.php?place=orchard&action=stand&pwd='+pwd)); break;
        case 3471:                                                              // damp old boot
            addWhere.append(Util.createLink('[old man, see?]','oldman.php')); break;
        case 4621:                                                                 // Game Grid Token
            addWhere.append(Util.createLink('[arcade]','arcade.php')); break;

        case 450:     case 451:     case 1258:                                        // Pretentious artist's stuff
            addWhere.append(Util.createLink('[artiste]',Util.getPlaceUrl('town_wrong&action=townwrong_artist_quest'))); break;

        case 4961:  case 4948:     case 4949:     case 4950:                            // subject 37 file, GOTO, weremoose spit, abominable blubber
            addWhere.append(Util.createLink('[visit 37]','cobbsknob.php?level=3&action=cell37')); break;
        case 5193:    case 5194:                                                    // 11-inch knob sausage, exorcised sandwich
            addWhere.append(Util.createLink('[back to the guild]','guild.php?place=challenge')); break;
        case 5221:                                                              // fat loot token
            addWhere.append(Util.createLink('[spend it!]','shop.php?whichshop=damachine',false)); break;
        case 1764:    case 7302:                                                    // spookyraven library key
            addWhere.append(Util.createLink('[library (1)]',Util.getSnarfblatUrl(390))); break;
        case 5570:                                                              // ninja carabiner
            addWhere.append(Util.createLink('[Open the Peak!]',Util.getPlaceUrl('mclargehuge&action=cloudypeak'))); break;
        case 5571:
            addWhere.append(Util.createLink('[visit the John]',Util.getPlaceUrl('mclargehuge&action=trappercabin'))); break;
        ...
        case 7262: case 7270:                                                   // I Love Me, 2 Love Me
            doWhat = "oneuse";
            //addWhere.append(Util.createLink('[The Dr is In!]',Util.getPlaceUrl('palindome&action=pal_droffice'))); break;
        case 4029:                                                                // Hyboria: memory of a grappling hook
            if (Character.get("Krakrox") == "A") {
    ...
    switch (doWhat) {
        case "equip":
            addWhere.append(Util.createLink('[equip]', equip({i:itemNum})));
            break;

        case "equipacc":
            addWhere.append(Util.createLink('[equip]', equip({i:itemNum,s:3})));
            break;
        ...
        case "use":
            ...
            //    AppendUseBox(itemNum, 0, 0, formWhere.get(0));    //need to replace this function....
                ...
                addWhere.append(Util.createLink('[use]', 'multiuse.php?pwd=' +
        ...
        case "skill":
            ...
            //    AppendUseBox(itemNum, 1, 1, formWhere.get(0));
                ...
                addWhere.append(Util.createLink('[use]', 'inv_use.php?pwd='+ pwd +
        ...
        case "malus":
            addWhere.append(Util.createLink('[malus]', 'guild.php?place=malus'));
            break;

        default:
            if (doWhat) {
                addWhere.append(Util.createLink('['+ doWhat +']', doWhat+'.php'));
            }
    }
...
function RightClickHP(e) {
    let json = Character.get("hplist");
    if (json != undefined && json != "") {
        let num = 0;
        let quant = 0;
        let list = $.parseJSON(json);
        let order;
        let heal = Character.get("maxHP") - Character.get("currentHP");
        ...
        if (num > 0) {
            let url = 'skills.php?action=Skillz&whichskill='+num+"&quantity="+1+'&pwd='+pwd;
            GM_get(url, function(result) {
                document.location.reload();
            });
        }
...
function MakeMaxButton(textField, maxfunction) {
    ...
    let stizzyle = 'border: 1px solid black; border-left: 0px; padding: 0px;';
...
function SkillUseLimit(skillNum) {
    let limit = 999999; let min = 0; let max = 0;
    let safeLevel = GetPref('safemax');
    ...
    if (max != 0) {
        let hp = Character.get("maxHP") - Character.get("currentHP");
        switch(safeLevel) {
...
function Defaults(revert) {
    ...
    let menu1 = ['market;town_market.php','hermit;hermit.php',
...
function MakeOption(text, num, pref, opt1, opt2) {
    ...
    let prefVal = GetPref(pref);
    let select;

    let selectid = "_mrscript_opt_"+pref.replace(" ", "_");
...
function AddToTopOfMain(newElement,refDocument) {
    let fightElement = refDocument.evaluate('//b[contains(.,"Combat") and contains(.,"!")]/ancestor::tr[1]',refDocument,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue;
    if (fightElement) {
        fightElement.parentNode.insertBefore(newElement,fightElement);
    } else {
        let element = refDocument.getElementsByTagName("tr")[0];
...
// code for NS Lair spoilers borrowed shamelessly from Tard's NS Trainer v0.8
    ...
    let $monster = $("#monname");
    let monsterName = $monster.text();
    let monsterNameShort = monsterName.replace(/^an?\s/, '');
    let infight = Character.get("infight");
    ...
    $('option[value="2947"]').each(function(){
        let insultsList = Character.get("insults"); if (insultsList == undefined) insultsList = "0;0;0;0;0;0;0;0";
        let insultsArray = insultsList.split(";");
        let numInsults = 0;
...
// PART 1: FIRST-ROUND STUFF
    if (infight != "Y") {    // first time through this particular fight?
        ...
        let monsterItem = MonsterArray[monsterName];
        ...
        if (monsterItem != undefined) {    // let's do something specific with this critter.
            let dropdown = document.getElementsByName('whichitem');
...
// PART 2: SPECIAL-PROCESS STUFF
    if (Character.get("special") != 0) {    // in a fight with something special?
        switch (Character.get("special"))
        {
            case 1:    // gremlins
...
// PART 3: LAST-ROUND STUFF
    ...
    if (/WINWINW/.test(document.body.innerHTML)) {
        ...
        let square=Character.get("square");
        SetCharData("square",false);
        //special: friar check:
        if (Character.get("hasflorist") === true) {
            if ($('img[src*=friarplant]').length == 0) {
                $('#monpic').parent().parent()
                    .append('<td><a href="'+Util.getPlaceUrl('forestvillage&action=fv_friar')+'">Flower Power!</a></td>');
            }
        }
...
// PART 4: ANY-ROUND STUFF
    // yoinked-item processing
        ...
        for (var i = 0; i < imgs.length; i++)
        {
            ...
            let text = img.parentNode.parentNode.parentNode.parentNode.parentNode.innerHTML;
            text = text.replace(/ acquire /, " yoinked ");

            let yoinked = Character.get("yoink");
...
function CheckLocation (square) {
    if (square.indexOf("cellar.php") != -1) {    // add "explore L/R/U/D" links
        ...
        let location = integer(square.match(/(\d+)/)[1]);    // the 185 in "adventure.php?snarfblat=185"
        switch (location)    {
            case 182:
            case 183:
            case 184:
...
function ProcessGremlinCombat(monsterName) {
    ...
    let gremlininfo    = {    "a batwinged gremlin":[182, "whips out a hammer",             "a bombing run over"],
    ...
    let zonetext = Character.get("square");    // should have something like "adventure.php?snarfblat=182"
    let zone = zonetext ? integer(zonetext.match(/(\d+)/)[1]) : 0;
    ...
    if ((zone && (gremlininfo[monsterName][0] != zone)) ||
        ...
        if (document.body.innerHTML.indexOf(gremlininfo[monsterName][1]) != -1) {    // and there it is!
            ...
            let itemSelect = document.getElementsByName('whichitem');
            let funkSelect = document.getElementsByName('whichitem2');
...
function ProcessNunMeatDrops() {
    ...
    let meatline = $("img[src*='meat.gif']:first").parent().next().text();    // should be "You gain X meat"
    if (/Meat\./.test(meatline)) {
        GM_log("Meatline was: "+ meatline);
        GM_log("found a dot, trying again....");
        meatline = $("img[src*='meat.gif']:last").parent().next().text();
        GM_log("new Meatline is: " +meatline);
    }
    let meat = integer(meatline.match(/You gain (\d+) Meat/)[1]);
    let meatSoFar = integer(Character.get("nunmoney"));
...
function ProcessWineCellarDrops() {
    let winebits = {"Merlot":1,"Marsala":2,"Muscat":4,"Pinot Noir":8,"Port":16,"Zinfandel":32};
    let imgs = document.getElementsByTagName('img');
    if (imgs.length > 1) {        // image 0 is the monster pic.  anything else might be a drop.
        let dropcode = 0;
        for (var i=1; i<imgs.length;i++) {
            let itemname = imgs[i].getAttribute('alt');
        ...
        if (dropcode != 0) {
            let cref = $('a[href*="snarfblat"]').attr("href");
            let corner = "corner" + cref.match(/snarfblat=(\d+)/)[1];
            let winesfound = Character.get(corner);
            winesfound |= dropcode;
...
function link_cellar(square) {
    let thissq = square.match(/(\d+)/)[1]; // get number from "cellar.php?action=explore&whichspot=19"
    thissq = integer(thissq);
    let myhrefbase = "cellar.php?action=explore&whichspot=";
    let myhref = "";
    let UP = 8;
    let DOWN = 4;
    let LEFT = 2;
    let RIGHT = 1;
    let dtable=$('<table id="dirlinks"><tr>'+
                    '<td id=ul>&nbsp;</td>'+
                    '<td id=up><a>&nbsp;</a></td>'+
                    '<td id=ur>&nbsp;</td></tr>'+
    ...
    let grid = [0,5,7,7,6,0,13,15,15,15,6,13,15,15,15,14,13,15,15,15,14,9,11,11,11,10];
    let beenTo = Character.get("squarelist") + ";" ;
...
function showYoinks(wonCombat) {
    let yoink = Character.get("yoink");
    if (yoink == undefined) yoink = "";
    yoink = HTMLDecode(yoink);
    if (yoink != "") {
        SetCharData("yoink", "");
        let yoinkNode = document.createElement("table");
        yoinkNode.innerHTML = yoink;
        let rel = yoink.match(/rel="([^">]*)">/)[1];
        $(yoinkNode).attr('class','item').attr('style','float: none').attr('rel',rel);
        if (wonCombat) {
            let centers = document.body.getElementsByTagName("center");
...
function at_cove() {
    let insultsList=Character.get("insults");
    ...
    let iColor={0:"red",1:"red",2:"red",3:"red",4:"red",5:"maroon",6:"blue",7:"green",8:"green"};

    //Create the page element
    let InsultDisplay = document.createElement('tr');
...
function cellar_linker() {
    let a = $(this);
    SetCharData("square", a.attr('href'));
    let squarenum = a.attr('href').match(/(\d+)/)[0];
    let sqlist = Character.get("squarelist");
...
function at_cobbsknob() {
    $('p').each(function() {
        ...
        if ((txt.indexOf("I'll give it a shot") != -1) ||
            (txt.indexOf("I'll get right on it") != -1))  { $(this).append(Util.createLink('[lab (1)]',Util.getSnarfblatUrl(50))); }
        if ((txt.indexOf("them on Menagerie level 1") != -1) ||
            (txt.indexOf("First, I'm going to need") != -1)) { $(this).append(Util.createLink('[menagerie-1 (1)]',Util.getSnarfblatUrl(51))); }
        else if ((txt.indexOf("Okay, the next thing") != -1) ||
            (txt.indexOf("flartble") != -1)) { $(this).append(Util.createLink('[menagerie-2 (1)]',Util.getSnarfblatUrl(52))); }
        else if ((txt.indexOf("This is the last thing I need") != -1) ||
            (txt.indexOf("be right back") != -1)) { $(this).append(Util.createLink('[menagerie-3 (1)]',Util.getSnarfblatUrl(53))); }
    });
    // stupid not-<p>-tagged option...
    $("td:contains('How embarrassing.'):last").append(Util.createLink('[menagerie-2 (1)]',Util.getSnarfblatUrl(52)));
}
...
//        if (square.indexOf("cellar.php") != -1) link_cellar(square);
    ...
    let $NCTitle = $('b:eq(1)');
    let NCText = $NCTitle.text();
//    console.log("NCTtext=["+$NCTitle.text()+"]");
    switch (NCText) {
    case "Rotting Matilda":
        let cardlink = document.createElement('table');
...
//        SetCharData("altar4",'');
    ...
    case "3 eXXXtreme 4ever 6pack":
        $('<center><a href="'+Util.getPlaceUrl('mclargehuge&action=cloudypeak') + '">Open the Peak!</a></center><br />').appendTo($('p:last').parent());
        break;
    case "The Journey Continues":
        let p1 = $('p:contains("Having finally fought")').text;
    ...
    case "Top of the Castle, Ma":
        $('<center><a href="'+Util.getSnarfblatUrl(324)+'">Adventure on the TOP FLOOR</a></center><br />').prependTo($('a:last').parent());
        break;
    case "Plucked":
    ...
    case "Not So Much With The Humanity":
        $('<center><a href="'+Util.getSnarfblatUrl(385)+'">Adventure in the Red Zeppelin</a></center><br />').prependTo($('a:last').parent());
        break;
    case "":    // got a "You shouldn't be here" or other reject message... (or an untitled Limerick Dungeon adventure, sigh)
        if (document.location.search=="?snarfblat=100") {    // we were trying for Whitey's Grove; go get the quest from the guild.
            ...
            $('<center><a href="'+Util.getSnarfblatUrl(323)+'">Work the Ground Floor Some More</a></center><br />')
                .prependTo($('a:last').parent());
        }
        break;
    }
...
function at_guild() {
    ...
    let subloc = document.location.search;
    let td = $('center table tr td center table tr td:first');
    let tdtext = $(td).text();
    switch (subloc) {
        case "?place=paco":
            if (tdtext.indexOf("White Citadel") != -1) {
                td.append(Util.createLink('[Whitey\'s Grove (1)]', Util.getSnarfblatUrl(100)));
                td.append(Util.createLink('[Road to WC (1)]', Util.getSnarfblatUrl(413)));
            }
            else if ((tdtext.indexOf("with the meatcar?") != -1) || (tdtext.indexOf("meatcar parts") != -1)) {
                if (Character.get("friendlyknoll") == true) {
                    td.append(Util.createLink('[gnoll store]', "store.php?whichstore=5"));
                } else {
                     td.append(Util.createLink('[Knoll]', Util.getPlaceUrl('knoll_hostile')));
                }
            }
        break;
        case "?place=ocg":
            if (tdtext.indexOf("Misspelled Cemetary") != -1) {    // common opening phrase to find F's key.
                $('p:last').append(Util.createLink('[Misspelled Cemetary (1)]',Util.getSnarfblatUrl(21)));
            ...
            {
                td.append(Util.createLink('[Misspelled Cemetary (1)]',Util.getSnarfblatUrl(21)));
            ...
            {
                td.append(Util.createLink('[Tower Ruins (1)]',Util.getSnarfblatUrl(22)));
            }
        break;
        case "?place=scg":
            if (tdtext.indexOf("the two oldest and wisest") != -1)         // common opening phrase, yay.
            {
                $('p:last').append(Util.createLink('[hermit]','hermit.php')).append(Util.createLink('[casino]','casino.php'));
            ...
            {
                td.append(Util.createLink('[hermit]','hermit.php')).append(Util.createLink('[casino]','casino.php'));
            } else if (tdtext.indexOf("Beelzebozo") != -1)                         // EW->LEW assignment, all classes.
            {
                $('p:last').append(Util.createLink('[Fun House (1)]',Util.getSnarfblatUrl(20)));
            ...
            {
                td.append(Util.createLink('[Fun House (1)]',Util.getSnarfblatUrl(20)));
            } else if (tdtext.indexOf("on your map") != -1)                 // Cave assignment, all classes
            {
                if ($('p').length) $('p:last').append(Util.createLink('[nemesis cave]','cave.php'));
                else td.append(Util.createLink('[nemesis cave]','cave.php'));
            ...
            {
                td.append(Util.createLink('[nemesis cave]','cave.php'));
                ...
                $('p:last').append(Util.createLink('[equip fledges]',equip({i:3033,s:3})))
                           .append(Util.createLink('[Poop Deck (1)]',Util.getSnarfblatUrl(159)));
            }
        break;
        case "?place=challenge":
            ...
            if ((tdtext.indexOf("So you wanna join the Department of Shadowy") != -1) ||    // moxie quest opener
                ...
                $('p:last').append(Util.createLink('[back alley (1)]',Util.getSnarfblatUrl(112)));
                ...
                $('p:last').append(Util.createLink('[knob outskirts (1)]',Util.getSnarfblatUrl(114)));
            } else if ((tdtext.indexOf("particular poltersandwich") != -1) ||               // myst opener
                        (tdtext.indexOf("cinnabonshee") != -1)) {                           // myst not-done-yet
                $('p:last').append(Util.createLink('[haunted pantry (1)]',Util.getSnarfblatUrl(113)));
            }
        break;
    }
}
...
function at_arcade() {
    GM_get('api.php?what=inventory&for=MrScript',function(response) {
        let invcache = $.parseJSON(response);
        let tokens = ((invcache[4621] === undefined) ? "no" : invcache[4621]) + " token" + ((invcache[4621] == 1) ? " " : "s ");
        let tickets = ((invcache[4622] === undefined) ? "no" : invcache[4622]) + " ticket" + ((invcache[4622] == 1) ? ". " : "s. ");
        let arcadeInfo = document.createElement('div');
...
function at_choice() {
    ...
    let inputs = $('input[name="whichchoice"]');
    ...
    let choicetext = $('body').text(); // for finding stuff that's not in a <p> tag.  sigh.
...
//    let p=document.getElementsByTagName('p');
    if (p.length) {
        ...
        if (p0text.indexOf("actually a book.") != -1) {    // The Oracle
            ...
            p0.appendChild(Util.createLink('[back to the guild]','guild.php?place=challenge'));
            ...
            let saidgmob = Character.get("saidbeeguy");
        ...
        {
            $('<center><a href="'+Util.getSnarfblatUrl(323)+'">Adventure on the GROUND FLOOR</a></center><br />')
                .prependTo($('a:contains("Adventure Again")').parent());
        } else if (p0text.indexOf("The ground floor is lit much better") != -1) {
            $('<center><a href="'+Util.getSnarfblatUrl(324)+'">Adventure on the TOP FLOOR</a></center><br />')
            ...
            $('<center><a href="'+Util.getPlaceUrl('hiddencity')+'">Visit Beautiful Downtown Hidden City</a></center><br />')
                .prependTo($('a:contains("Go back to")').parent());
            //    p0.appendChild(Util.createLink('[hidden city]',Util.getPlaceUrl('hiddencity')));
        } else if (choicetext.indexOf("hacienda. You do find an") != -1) {
            let clueText = /You do find an (\.*?),/.match(choicetext)[1];
            ...
            let kText = [["The lever slides down and stops","L"],
...
function at_forestvillage() {
    let plunger = Character.get("friendlyknoll");
    let linkloc = plunger ? Util.getPlaceUrl("knoll_friendly&action=dk_innabox") : Util.getPlaceUrl('knoll_hostile');
    let linkname = plunger ? "[get it from Innabox]" : "[head to degrassi knoll]";
    $('td:contains("just lost without my"):last').append(Util.createLink(linkname,linkloc));
    $('td:contains("luck finding my screw"):last').append(Util.createLink(linkname,linkloc));
    if (plunger) $('b:contains("Untinker")').append(Util.createLink('[innabox]',linkloc));
}
...
function at_bounty() {
    ...
    $('img[width=30][class!=hand]').each(function(n, el) {
        let $img = $(this); if($img.attr("title") === "filthy lucre") return true;
        let $td = $img.parent().parent().children("td:eq(1)");

        let txt = $td.text();
        if(txt) for (var i=0; i<bountyloc.length; i++) {
            if (txt.indexOf(bountyloc[i][0]) !== -1) {
                $td.append(Util.createLink(bountyloc[i][1], Util.getSnarfblatUrl(bountyloc[i][2])));
                break;
            }
        }
    });
...
/*
    $('p:first').each(function() {
        ...
        for (var i=0; i<bountyloc.length; i++) {
            if (txt.indexOf(bountyloc[i][0]) !== -1) {
                p.append(Util.createLink(bountyloc[i][1],Util.getSnarfblatUrl(bountyloc[i][2])));
                break;
            }
        }
    });
...
function at_beerpong() {
    let val = 0, html = $('img[src*="beerpong"]').parent().parent().html();
    if (html) {
        ...
        let sel = $('select[name="response"]');
        sel.children().each(function() {
            if ($(this).val() > 8) $(this).attr('disabled','disabled');
        });
        if (val > 0) {
            let opt = sel.find('option[value="'+val+'"]');
    ...
    if ($('p:first').text().indexOf("You laugh as Ricket") != -1) {        // insert only upon beerpong success.
        $('a[href="cove.php"]').parent().prepend("<center><a href='"+Util.getSnarfblatUrl(158)+"'>Adventure in the F'c'le</a></center><br />");
    }
}
...
function at_inventory() {
    ...
    let gearpage = 0; // Man, this is annoying.
    ...
    // Miscellaneous messages that always route you back to inventory:
        ...
        if (src.indexOf('blackbird1') != -1) {                                    // blackbird
            ...
            if (clov.length > 0) {
                let quant = clov.text().match(/^[0-9]*/);
                if (!quant) quant = 1;
                clov.append(Util.createLink('[disassemble]','multiuse.php?pwd='+
...
//    if(GetPref("inlineitemfilter"))
    $("#filter input").on("keyup", function(e) {
        let $input = $(this);
        let filterVal = $input.val().toLowerCase();
    ...
    if (gearpage == 1) {
        let backup = GetPref('backup');
        let quickequip = GetPref("quickequip");
        let lnks = document.links;
        let unlink, famLock;
        let didQElink = false;
        let selecty = document.getElementsByTagName('select')[0];
                 ...
                 || lnk.text == "Manage your Custom Outfits")
                {
                    ...
                    if (processingUnequipAll == 1) {
                        ...
                        newlink.addEventListener('click',function(event) {
                            ...
                            GM_get(equip({a:'customoutfit',oname:GetPref('backup')}),
...
// this is where we go back to a useful location if we've done/used something elsewhere that caused the inventory page to load.
    if (GetPref('shortlinks') > 1 && firstTable.rows[0].textContent == "Results:") {
        let resultsText = firstTable.rows[1].textContent;
        let jb = $('b:eq(1)');
        let bText = jb.text();
        let goHere = checkForRedirects(resultsText);
        if (goHere != "") {
            mainpane_goto(goHere);
        }
// and this is where we add all the nifty little links after equipping something.
            ...
            let theItem = jb.parent().parent().get(0);
            AddLinks(null, theItem, null, thePath);
        }
...
function process_results(rText, insLoc) {
    if (rText.indexOf("You can easily climb the branches") != -1) {
        insLoc.append(Util.createLink('[temple (1)]',Util.getSnarfblatUrl(280)));
    } else if (rText.indexOf("You should go to A-Boo Peak") != -1) {
        if (insLoc.parent().find('p').length === 0) {
            insLoc.append(Util.createLink('[A-Boo! (1)]',Util.getSnarfblatUrl(296)));
        } else {
            insLoc.parent().find('p').append(Util.createLink('[A-Boo! (1)]',Util.getSnarfblatUrl(296)));
        }
    } else if (rText.indexOf("You give the wheel a mighty turn") != -1) {
        mainpane_goto('pyramid.php');
    } else if (rText.indexOf("it just seemed like a cool spy thing") != -1) {
        insLoc.append(Util.createLink('[venture into the Knob]','cobbsknob.php'));
    } else if (rText.indexOf("At least now you know where the pyramid is") != -1) {
        insLoc.append(Util.createLink('[$64,000 pyramid, baby]',Util.getPlaceUrl('desertbeach&action=db_pyramid1')));
    } else if (rText.indexOf("named Mr. Alarm that Dr. Awkward") != -1) {
        insLoc.append(Util.createLink('[knob lab (1)',Util.getSnarfblatUrl(50)));
    } else if (rText.indexOf("insane, egotistical ramblings") != -1) {
        insLoc.append(Util.createLink('[Go to Dr, do tog]',Util.getPlaceUrl('palindome&action=pal_droffice')));
    } else if (rText.indexOf('second volume is more of the same') != -1) {
        insLoc.append(Util.createLink('[Go see Mr Alarm, ee! sog.]', Util.getPlaceUrl('palindome&action=pal_mroffice')));
    }
}
...
function at_palindome() {
    if ($('body').text().indexOf("accessible to you") != -1) {
        $('td:contains("That place"):last')
            .append(Util.createLink('[equip Talisman, o Nam, silat! pi uqe?]',equip({i:486,s:3})));
    }
}
...
function at_galaktik() {
    ...
    let row = $('table:first tr:eq(1):contains("You acquire")'), txt;
    if (row.length == 1) {
        let num = 1;
        txt = row.text();
        if (txt.indexOf("an item:") == -1)
            num = $('b:eq(1)').text().split(" ")[0];
        let docG = integer($('table.item').attr('rel').match(/id=(\d+)/)[1]);
        if (GetPref('docuse') == 1 && docG < 233) {    // 231=unguent, 232=ointment.  we can auto-use those.
            let sanitycheck = FindMaxQuantity(docG, num, 0, 0) + 1;
    ...
    let howMany = $('input[name="howmany"]');
    ...
    check.change(function() {
        let box = $('input[name="usecheckbox"]');
...
//        select[0].options.selectedIndex = 1;
        let qty = document.getElementsByName("quantity")[0];
        MakeMaxButton(qty, function(event) {
            let selectItem = document.getElementsByName('whichitem')[0];
            let box = document.getElementsByName('quantity')[0];
            let quant = ParseSelectQuantity(selectItem, ")");
            box.value = quant;
        });
    }
}
...
function pants(evt) {
    GM_get(equip({i:1792}), //inv_equip.php?pwd='+pwd+'&which=2&action=equip&whichitem=1792'
    function(txt) {
        let pimg = document.getElementById('proprietor');
...
function at_store() {
    ...
    let firstTable = $('table:first tbody');        // we're interested in this when it's the "Results:" box from buying something.
    let whichstore;
    let noform = 1;

    let insput = $('input[name="whichstore"]');
    if (insput.length > 0) {
        whichstore = insput.attr('value');
        noform = 0;
    } else {
        insput = $('input[name="whichshop"]');
        if (insput.length > 0) {
            whichstore = insput.attr('value');
            noform = 0;
        }
    ...
    {
        let descId = $('img:first').get(0).getAttribute('onclick');
        let acquireString = firstTable.children('tr:eq(1)').text();
        let acquireText = firstTable.find('tr:eq(1) td:first *:first');
        let bText = $('b:eq(1)').attr('valign','baseline');
        switch(whichstore) {
            case 'b':        // everything from the bugbear bakery is cookable.
                bText.parent().append(Util.createLink('[cook]', '/craft.php?mode=cook')); break;
            case 'j':        // everything from the jeweler is pliable
                bText.parent().append(Util.createLink('[ply]', 'craft.php?mode=jewelry'));
                break;
            case 's':        // everything from the meatsmith is smithable.
                ...
                bText.parent().append(Util.createLink('[smith]', 'craft.php?mode=smith'));
                break;
            case 'h':        // everything from the hippy is cook/mix/stillable.
                bText.parent()
                    .append(Util.createLink('[cook]', 'craft.php?mode=cook'))
                    .append(Util.createLink('[mix]', 'craft.php?mode=cocktail'))
                    .append(Util.createLink('[still]', 'guild.php?place=still'));
                break;
            case 'r':        // pirate store: untinker the dictionary.
                if (acquireString.indexOf('dictionary') != -1)
                    bText.parent().append(Util.createLink('[untinker]', Util.getPlaceUrl('forestvillage&action=fv_untinker')));
                break;
        }

        if (descId != undefined) {
            let whut = AddLinks(descId, bText.parent().parent().get(0), acquireText, thePath);
...
function at_craft() {
    InlineItemDescriptions();
    let mode, mlink, store;
    let itemNeeded = 0, desc = "";
    mode = $('input[name="mode"]').val();
    let dtext = [    ["combine","meat pastables"],["cook","foods"],["smith","arms + armor"],
            ["cocktail","booze"],["jewelry","jewelry"],["multi","miscellaneous"]];
    let dlinks = "<tr><td><center><font size=2>Discoveries:&nbsp;";
    ...
    let tbltext = mlink.parents('table:first').text();
    ...
    if (GetPref('shortlinks') > 1 && mlink.length > 0 && ilink.length == 0 && itemNeeded > 0) {
        ...
        GM_get('heydeze.php', function(txt)
        {    if (txt != '') store = 'y';
            ...
            $('b').each(function() {
                ...
                if (txt.indexOf("powder") != -1) {
                    ...
                    if (txt.indexOf("twink") == -1)
                        bParent.append(Util.createLink('[cook]',
                        '/craft.php?mode=cook'));
                }
            });
            break;
...
function at_hermit() {
    $('img').each(function() {
        ...
        {
            let newclick = onclick.split(':')[0] + ":desc" + onclick.split(':')[1];
    ...
    if (GetPref('shortlinks') > 1) {
        ...
        if (txt.indexOf("Hermit Permit required,") != -1) {            // no permit
            ...
            GM_get('api.php?what=inventory&for=MrScript',function(response) {        // Check gum and permit counts.
                let invcache = $.parseJSON(response);
                let gum = invcache[23]; if (gum === undefined) gum = 0;
                gum = integer(gum);
                let hpermit = invcache[42]; if (hpermit === undefined) hpermit = 0;
...
//                    +" and "+(hpermit==0?" no ":hpermit)+(hpermit!=1?" permits ":" permit ")
        ...
        let tr = $('table:first tr:contains("Results")');
        if (tr.next().text().indexOf("You acquire") != -1) {
            let descId = $('img:first').get(0).getAttribute('onclick');
            let bText = $('b:eq(1)').attr('valign','baseline');
            if (bText.text().indexOf("ten-leaf clovers") != -1) {
                let num = integer(bText.text().split(" ten-leaf")[0]);
                bText.parent().append(Util.createLink('[disassemble]', 'multiuse.php?pwd=' +
...
function at_knoll_friendly() {
    if (document.location.search.indexOf("dk_mayor") != -1) {
        p = $('p:first');
        p.append(Util.createLink('[spooky barrow (1)]','adventure.php?snarfblat=48'));
        p.append(Util.createLink('[bugbear pens (1)]','adventure.php?snarfblat=47'));
    }
    let txt = $("tr:eq(1) > td:first > center tr:first").text();
    ...
    if (GetPref('shortlinks') > 1) {
        $('p').each(function() {
            ...
            if (txt.indexOf("Toot") != -1)
                p.append(Util.createLink('[toot]', 'tutorial.php?action=toot'));
            else if (txt.indexOf("larva") != -1 && txt.indexOf("Thanks") == -1)
                p.append(Util.createLink('[woods]', 'woods.php'));
            else if (txt.indexOf("Typical Tavern") != -1)
                p.append(Util.createLink('[Bart Ender]','tavern.php?place=barkeep'));
            else if (txt.indexOf("Boss Bat") != -1)
                p.append(Util.createLink('[bat hole]', Util.getPlaceUrl("bathole")));
            else if (txt.indexOf("Guild") != -1)
                p.append(Util.createLink('[guild]', 'guild.php'));
            else if (txt.indexOf("Goblin King") != -1 &&
                txt.indexOf("slaying") == -1)
            {
                let derr = Util.createLink('[harem outfit]', equip({a:'outfit',oid:4}));
                p.append(derr);
                if (GetPref('backup') != "") {
                    $(derr).children('*:last')
                ...
                derr = Util.createLink('[guard outfit]', "inv_equip.php" +
                ...
                p.append(Util.createLink('[cyrpt]', 'crypt.php'));
            else if (txt.indexOf("the Trapper") != -1)
                p.append(Util.createLink('[John, M.D.]', Util.getPlaceUrl('mclargehuge&action=trappercabin')));
            else if (txt.indexOf("Chasm") != -1)
                p.append(Util.createLink('[mountains]', 'mountains.php'));
            else if ((txt.indexOf("Highland Lord") != -1) || (txt.indexOf("Black Angus") != -1)) {
                p.append(Util.createLink('[chasm]',Util.getPlaceUrl('orc_chasm')));
                p.append(Util.createLink('[highlands]',Util.getPlaceUrl('highlands')));
            }
            if (txt.indexOf("invaded!") != -1 || txt.indexOf("pirates") != -1)
            {
                let derr = Util.createLink('[swashbuckle]',equip({a:'outfit',oid:9}));
                p.append(derr);
                if (GetPref('backup') != "") {
                ...
                p.append(Util.createLink('[island]', 'island.php'));
            }
            else if (txt.indexOf("garbage") != -1
                && txt.indexOf("Thanks") == -1)
            {
                if (txt.indexOf("sky") != -1) {
                    p.append(Util.createLink('[plant bean]',Util.getPlaceUrl('plains&action=garbage_grounds')));
                    top.frames[0].location.reload();
                } else p.append(Util.createLink('[beanstalk]', Util.getPlaceUrl('beanstalk')));
            }
            else if (txt.indexOf("her Lair") != -1)
                p.append(Util.createLink('[lair]', 'lair.php'));
            else if (txt.indexOf("Black Forest") != -1)
                p.append(Util.createLink('[forest (1)]', Util.getSnarfblatUrl(405)));
        ...
        $('b').each(function() {
            ...
            switch (txt) {
                case "Looking for a Larva in All the Wrong Places":
                    b.append(Util.createLink('[woods]', 'woods.php'));
                    break;
                case "Ooh, I Think I Smell a Bat.":
                    b.append(Util.createLink('[bat hole]', Util.getPlaceUrl("bathole")));
                    break;
                case "The Goblin Who Wouldn't Be King":
                    let derr = Util.createLink('[disguise]', "inv_equip.php?action=outfit&which=2&whichoutfit=4");
                    b.append(derr);
                    if (GetPref('backup') != "") {
                        $(derr).children('*:last')
                        .attr('href', 'javascript:void(0);').click(function() {
                    ...
                    break;
                case "Trial By Friar":
                    b.append(Util.createLink('[copse]', 'friars.php'));
                    break;
                case "Cyrptic Emanations":
                    b.append(Util.createLink('[cyrpt]', 'crypt.php'));
                    break;
                case "Am I My Trapper's Keeper?":
                    b.append(Util.createLink('[trapz0r]', 'trapper.php'));
                    break;
                case "A Quest, LOL":
                    b.append(Util.createLink('[mountains]', 'mountains.php'));
                    let derr = Util.createLink('[swashbuckle]', equip({a:'outfit',oid:9}));
                    b.append(derr);
                    if (GetPref('backup') != "") {
                        $(derr).children('*:last')
                    ...
                    b.append(Util.createLink('[island]', 'island.php'));
                    b.append(Util.createLink('[bookstore]', 'store.php?whichstore=r'));
                    break;
                case "The Rain on the Plains is Mainly Garbage":
                    b.append(Util.createLink('[beanstalk]', Util.getPlaceUrl('beanstalk')));
                    break;
                case "Never Odd Or Even":
                    let subtext = $(this).parent().text(); // contents().get(2).data;
                    ...
                    b.append(Util.createLink("[o' nam]", 'inv_equip.php?pwd='+pwd+
                        "&which=2&slot=3&action=equip&whichitem=486"));
                    b.append(Util.createLink('[palindome (1)]', Util.getSnarfblatUrl(119)));
                    b.append(Util.createLink('[poop deck (1)]', Util.getSnarfblatUrl(159)));
                    break;
                case "A Pyramid Scheme":
                    b.append(Util.createLink('[beach]', 'beach.php'));
                    break;
                case "Gotta Worship Them All":
                    b.append(Util.createLink('[hidden city]', 'hiddencity.php'));
                    break;
                case "In a Manor of Spooking":
                    b.append(Util.createLink('[ballroom (1)]', Util.getSnarfblatUrl(109)));
                    b.append(Util.createLink('[wine cellars]', 'manor3.php'));
                    break;
                case "Make War, Not... Oh, Wait":
                    b.append(Util.createLink("[pre-war island]","island.php"));
                    b.append(Util.createLink("[post-war island]","bigisland.php"));
                    break;
                case "Ooh, I Think I Small a Rat":
                    b.append(Util.createLink('[tavern]', 'cellar.php'));
                    break;
                case "My Other Car is Made of Meat":
                    b.append(Util.createLink('[untinker]', Util.getPlaceUrl('forestvillage&action=fv_untinker')));
                    b.append(Util.createLink('[plains]', Util.getPlaceUrl('plains')));
                    break;
                case "The Wizard of Ego":
                    b.append(Util.createLink('[plains]', Util.getPlaceUrl('plains')));
                    break;
                case "Me and My Nemesis":
                    let subtext = $(this).parent().text();
                    if (subtext == undefined) break;
                    if (subtext.indexOf("smith an Epic Weapon") != -1) {
                        b.append(Util.createLink("[casino]","town_wrong.php?place=casino"));
                        b.append(Util.createLink("[hermit]","hermit.php"));
                        b.append(Util.createLink("[smith]","craft.php?mode=smith"));
                    } else if (subtext.indexOf("defeat Beelzebobo") != -1)
                        b.append(Util.createLink("[funhouse (1)]",Util.getSnarfblatUrl(20)));
                    else if (subtext.indexOf("head back to your guild") != -1)
                        b.append(Util.createLink("[guild]","guild.php?place=scg"));
                    else if (subtext.indexOf("map to the secret") != -1)
                        b.append(Util.createLink("[poop deck (1)]",Util.getSnarfblatUrl(159)));
                    else if (subtext.indexOf("arrived at the secret") != -1)
                        b.append(Util.createLink("[volcano]","volcanoisland.php"));
                    else if (subtext.indexOf("Well, you defeated") != -1)
                        b.append(Util.createLink("[nemesis lair (1)]","adventure.php?snarfblat=??"));
                    else if (subtext.indexOf("It took long enough, jeez!") != -1)
                        b.append(Util.createLink("[cave]","cave.php"));
                    break;
                case "A Dark and Dank and Sinister Quest":
                    b.append(Util.createLink("[cave]","cave.php"));
                    break;
                case "What's Up, Doc?":
                    b.append(Util.createLink("[Harem (1)]",Util.getSnarfblatUrl(259)));
                    b.append(Util.createLink("[Elbow (1)]",Util.getSnarfblatUrl(0)));
                    b.append(Util.createLink("[Slums (1)]",Util.getSnarfblatUrl(248)));
                    b.append(Util.createLink("[Cemetery (1)]",Util.getSnarfblatUrl(58)));
                    break;
                case "Driven Crazy":
                    b.append(Util.createLink('[plains]', Util.getPlaceUrl('plains')));
                    break;
                case "The Final Ultimate Epic Final Conflict":
                    ...
                    b.append(Util.createLink('[lair]', 'lair.php'));
                    break;
...
//                case "Go To White Citadel":
                case "I Rate, You Rate":
                    ...
                    if (GetPref('backup') != "")
                    {
                        ...
                        .click(function(event)
                        {
                            GM_get( equip({a:'customoutfit',oname:GetPref('backup')}), function(response)
                            {   parent.frames[2].location = 'http://'+server + equip({a:'outfit',oid:9})
                            }); return false;
                            //event.stopPropagation(); event.preventDefault();
                        });
                    }
                    b.append(Util.createLink('[cove]', 'cove.php'));
                    break;
                case "Toot!":
                    b.append(Util.createLink('[toot]', 'tutorial.php?action=toot'));
                    break;
                case "When Rocks Attack":
                    b.append(Util.createLink('[Doc G]', 'galaktik.php'));
                    break;
                case "Suffering For His Art":
                    b.append(Util.createLink('[artist]', 'town_wrong.php?place=artist'));
                    break;
                case "A Bugbear of a Problem":
                    b.append(Util.createLink('[bugbear pens (1)]',Util.getSnarfblatUrl(47)));
                    b.append(Util.createLink('[barrow (1)]', Util.getSnarfblatUrl(48)));
                    b.append(Util.createLink('[shrooms]', 'knoll_mushrooms.php'));
                    break;
                case "Out of Your Gourd":
                    let subtext = $(this).parent().text();
                    if (subtext.indexOf("Back Alley") != -1)
                        b.append(Util.createLink('[alley (1)]', Util.getSnarfblatUrl(112)));
                    else if (subtext.indexOf("Haunted Pantry") != -1)
                        b.append(Util.createLink('[pantry (1)]', Util.getSnarfblatUrl(113)));
                    else if (subtext.indexOf("Outskirts") != -1)
                        b.append(Util.createLink('[outskirts (1)]', Util.getSnarfblatUrl(114)));
                    break;
                case "An Old Guy and The Ocean":
                    b.append(Util.createLink("[octopus' garden (1)]",Util.getSnarfblatUrl(190)));
                    b.append(Util.createLink("[wreck (1)]",Util.getSnarfblatUrl(191)));
                    b.append(Util.createLink("[sea monkey castle]","monkeycastle.php"));
                    break;
                ...
                case "Primordial Fear":
                    b.append(Util.createLink('[soup (1)]', Util.getSnarfblatUrl(204)));
                    break;
                case "Hyboria? I don't even...":
                    b.append(Util.createLink('[jungle (1)]', Util.getSnarfblatUrl(205)));
                    break;
                case "Future":
                    b.append(Util.createLink('[future (1)]', Util.getSnarfblatUrl(206)));
                    break;
                case "Hammer Time":
                    b.append(Util.createLink('[alley (1)]', Util.getSnarfblatUrl(112)));
                    break;
                ...
                case "The Pretty Good Escape":
                    let subtext = $(this).parent().text();
                    if      (subtext.indexOf("level 1")) b.append(Util.createLink("[level 1 (1)]",Util.getSnarfblatUrl(51)));
                    else if (subtext.indexOf("Level 2")) b.append(Util.createLink("[level 2 (1)]",Util.getSnarfblatUrl(52)));
                    else if (subtext.indexOf("Level 3")) b.append(Util.createLink("[level 3 (1)]",Util.getSnarfblatUrl(53)));
                    else if (subtext.indexOf("ject 37")) b.append(Util.createLink("[subject 37]","cobbsknob.php?level=3&action=cell37"));
                    break;
...
// Here we put all the quests that have variable text in their labels, like %Playername.
            if (txt.indexOf("White Citadel") != -1)
                ...
                b.append(Util.createLink('[gate (1)]', Util.getSnarfblatUrl(79)));
            else if (txt.indexOf("Quest for the Holy MacGuffin") != -1) {
                let subtext = $(this).parent().text(); //contents().get(2).data;
                if (subtext.indexOf("find the Black Market") != -1)
                    b.append(Util.createLink("[black forest (1)]",Util.getSnarfblatUrl(405)));
                else if (subtext.indexOf("now to hit the Travel Agency") != -1)
                    b.append(Util.createLink("[shore]",Util.getPlaceUrl('desertbeach')));
            }
            else if (txt.indexOf("Delivery Service") != -1) {
                b.append(Util.createLink("[guild]","guild.php?place=paco"));
...
function CompressThrallAndFamiliar(compressLevel) {
    let weight = 0;
    let chatlink = 'http://' + server + '/main.php?talktoreanimator=1';
    let reanimator = false;
    let $fp1 = $('.familiarpick:first');
    let $fp2 = $('.familiarpick:eq(1)');
    let fInfo = $fp2.siblings().text();
    if (!fInfo) return;         // no familiar info found
    if (fInfo.indexOf("(chat)") != -1) { reanimator = true; }
    ...
    if (compressLevel == 2) {   //show weight and XP only
        ...
        $fp2.next('font')
            ...
            .appendTo($fp1);
        $fp2.hide();
        if (reanimator) $fp1.append(Util.createLink('chat!',chatlink));
    } else {                    //show nothing
        $fp2.parent().hide();
    }
    $fp1.appendTo('b:contains(Familiar)').attr('title',fInfo);

    let tParent = $('b:contains(Thrall)').parent();

    tParent.siblings('font').children('b').text('');    //nuke the name
    let thrallInfo = tParent.siblings('font').text();
...
function at_charpane() {
    // let centerThing = document.getElementsByTagName('center');
    let imgs = document.images;
    if (imgs.length == 0 || imgs == null) return;
    let compactMode = imgs[0].getAttribute('height') < 60 || imgs[0].getAttribute('src').indexOf("clancy") > -1;
//if clancy is the first image, we're in compact mode, but he's 100 pixels tall, the little bastard.
    let bText = document.getElementsByTagName('b');
    let curHP, maxHP, curMP, maxMP, level, str, advcount, effLink;
    let oldcount = integer(Character.get('advcount'));
    let effectsDB = {
    'd33505':3,        // confused
...
//    SetData("charname",bText[0].textContent);
    let compressfam = GetPref("compressfam");
    if (compressfam > 0) {
    ...
    if (compactMode) {
        ...
        let lvlblock = $("center:contains('Lvl.'):first").text();    // this text is always present in compact mode
        ...
        function parse_cur_and_max(names, data) {
            for each (var name in names) {
                let cur_max = data.shift().split('/').map(integer);
                SetCharData("current"+ name, cur_max[0]);
                SetCharData("max"    + name, cur_max[1]);
            }
        }
        let lvlblock = $("td:contains('Level'):first").text();
        if (lvlblock) {
        ...
        let $statusblock = $('img[alt*="Hit Points"]').parent().parent().parent().parent();
        $statusblock.wrap('<div id="statusblock></div>');
        let $statusitems = $statusblock.find('span.black, span.red');
        let data = $.makeArray($statusitems.slice(0, 4)).map(text);
        ...
        if (GetPref('backup')) {
            ...
            {    GM_get( 'inv_equip.php?action=customoutfit&which=2&outfitname=' +
                GetPref('backup'),function(response)
                {    let msg; if (response.indexOf("custom outfits") == -1) msg = "Outfit Backed Up";
        ...
        let bEff = $('b:gt(4):contains(Effects)');
...
//    $('#nudges').hide();
    ...
    $('#nudgeblock').click(function() {
        $('#nudges').toggle();
        if ($('#nudges').is(':visible')) {
            $('#nbhint').text('click to hide')
        } else {
            $('#nbhint').text('click to show')
        }
    });

    // Re-hydrate (0)
    let temphydr = integer(Character.get('hydrate'));
    ...
    for (i=0,len=imgs.length; i<len; i++) {
        let img = imgs[i], imgClick = img.getAttribute('onclick');
        let imgSrc = img.src.substr(img.src.lastIndexOf('/')+1);
        if (imgSrc == 'mp.gif')
        ...
        let effName = (compactMode ? img.getAttribute('title') : img.parentNode.nextSibling.firstChild.innerHTML);
        ...
        if (imgSrc == 'poison.gif')    {
            ...
            img.addEventListener('contextmenu', function(event)    {
                ...
                GM_get('api.php?what=inventory&for=MrScript',function(response) {            // see if we have some anti-anti-antidote onhand
                    let invcache = $.parseJSON(response); //eval('('+response+')');
        ...
        else // if (img.getAttribute('oncontextmenu') == null)
        {
            ...
            let effNum = effectsDB[imgClick.substr(5,6)];
            if (effNum == undefined) continue;
            switch (effNum) {
                case 275: // hydrated
                    let hydtxt = img.parentNode.nextSibling.textContent;
                ...
                case 357:    // absinthe-minded: link to wormwood; light up on 9/5/1 turns left.
                    let abstxt = img.parentNode.nextSibling.textContent;
                    let fontA, fontB;
                    ...
                    img.parentNode.nextSibling.innerHTML = '<a target=mainpane href="' +
                        Util.getPlaceUrl("wormwood") + '">' + fontA + '<b>' +
                ...
                case 510:
                    ...
                    break;
                case 725:
                    $(img).parent().next().children('font').wrap('<a target=mainpane href="'+Util.getPlaceUrl('rabbithole') + '"/>');
                    break;
                case 549:
                    let fishyturns = img.parentNode.nextSibling.textContent;
                ...
                case 189: case 190: case 191: case 192: case 193:
                    ...
                    break;
                case 846:
                    let transtext = img.parentNode.nextSibling.textContent;
                    img.parentNode.nextSibling.innerHTML = '<a target=mainpane href="' + Util.getPlaceUrl('spaaace&arrive=1') +
...
function at_skills() {
    let miniSkills = document.location.search.indexOf("tiny=1") != -1;
    let inputStuff = document.getElementsByTagName("input");
    let noDisable = GetPref('nodisable');
    ...
    let vich = document.getElementsByName("whichitem");        // the MP-restorers item dropdown.
    if (vich[0] != undefined) {
        let json = "{"; let opt = vich[0].childNodes;
        for (i=0, len=opt.length; i<len; i++) {
            let optval = opt[i].value; let temp;
    ...
    for (var i=0, len=inputStuff.length; i<len; i++) {
        ...
        if (temp.value == "1" && temp.name == "quantity") {
            temp.addEventListener('keydown', function(event) {
                ...
                {
                    let selectItem = document.getElementsByName('whichskill')[0];
                    let cost = ParseSelectQuantity(selectItem, " ");
                    let limit = SkillUseLimit(selectItem.options[selectItem.selectedIndex].value);
                    let val = integer(Character.get("currentMP") / cost);
        ...
        if (temp.value == "1" && temp.name == "bufftimes") {
            ...
            temp.addEventListener('keydown', function(event) {
                ...
                {
                    let selectItem = document.getElementsByName('whichskill')[1];
                    let cost = ParseSelectQuantity(selectItem, " ");
                    this.value = integer(Character.get("currentMP") / cost);
                    event.stopPropagation(); event.preventDefault();
                }
            }, true);
            MakeMaxButton(temp, function(event) {
                let selectItem = document.getElementsByName('whichskill')[1];
                let box = document.getElementsByName('bufftimes')[0];
                let cost = ParseSelectQuantity(selectItem, " ");
                box.value = integer(Character.get("currentMP") / cost);
            });
        }
        ...
        if (temp.value == "1" && temp.name == "itemquantity") {
            temp.addEventListener('keyup', function(event) {
                if (event.which == 77 || event.which == 88 || event.which == 72) // 77 = 'm', 88 = 'x', 72 = 'h'
                {
                    ...
                    let quant = ParseSelectQuantity(selectItem, ")");
                    let index = selectItem.selectedIndex;
                    let val = FindMaxQuantity(selectItem.options[index].value, quant, 0, GetPref('safemax'));
                    if (event.which == 72) val = integer(val/2); // half
                    this.value = val;
                    event.stopPropagation(); event.preventDefault();
                }
            ...
            if (!miniSkills) {
                MakeMaxButton(temp, function(event) {
                    let selectItem = document.getElementsByName('whichitem')[0];
                    let index = selectItem.selectedIndex;
                    let box = document.getElementsByName('itemquantity')[0];
                    let quant = ParseSelectQuantity(selectItem, ")");
...
function at_clan_rumpus() {
    if (document.location.search == "?action=click&spot=3&furni=3" && GetPref('klaw') == 1) {
        let tr = $('table:first tr:first:contains("Results")');
        if (tr.length > 0) {
            txt = tr.next().text();
...
function at_charsheet() {
    ...
    let mystBonus = 0;
    let HPMPstats = $("table table table td:lt(10)").text();
    if (HPMPstats.indexOf("Mana Points") != -1) mystBonus = 5;

    $('td:contains("Protection"):not(:has(table))').each(function() {
        let pstring = $(this).next().text();
        let resistance = 0;
        let ProtNum = 0;
        let PLevels = ["Very Low","Low","Moderate","Considerable"];
        for (var j = 0; j < 4; j++) {
...
function at_thesea() {
    if (document.body.textContent.indexOf("Uh Oh!") != -1)
        mainpane_goto(Util.getPlaceUrl("sea_oldman&action=oldman_oldman"));
...
function at_pandamonium() {
    ...
    if (document.location.search=='?action=sven')    {
        let bognort = 0, stinkface = 0, flargwurm = 0, jim = 0;
        let pandasolved = Character.get("pandabandsolved");
        if ((pandasolved == false) || (pandasolved == undefined)) {    // no solution found yet?
            let itemlist = $('select[name="togive"]');
            let set1 = 0, set2 = 0;
            let bear = false, paper = false, marsh = false;
            let cake = false, cherry = false, pillow = false;
            itemlist.children().each(function(){
                let myval = integer(this.value);
                switch (myval) {
            ...
            bognort = Character.get("bognort");
            stinkface = Character.get("stinkface");
            flargwurm = Character.get("flargwurm");
            jim = Character.get("jim");
            $('form[name="bandcamp"]').prepend('<p><font color="blue">Solution found!  Click [Give It]!</font></p>');
            let members = $('select[name="bandmember"]');
            let items = $('select[name="togive"]').get(0);
// this is ugly-looking, but solid.
        ...
        $('input.button[value*="Insult"]').parent().append(Util.createLink('[put on Victor (offhand)]','inv_equip.php?pwd='+pwd+'&which=2&action=equip&whichitem=4667'));
        $('input.button[value*="Observational"]').parent().append(Util.createLink('[put on observational glasses (acc1)]','inv_equip.php?pwd='+pwd+'&which=2&action=equip&whichitem=4668&slot=1'));
        $('input.button[value*="Comedy"]').parent().append(Util.createLink('[put on hilarious comedy Prop (weapon)]','inv_equip.php?pwd='+pwd+'&which=2&action=equip&whichitem=4669'));
    }
}
...
function at_pyramid() {
    ...
    checkInv.addEventListener('click', function(evt) {
        ...
        GM_get('api.php?what=inventory&for=MrScript',function(response) {
            if (response[0] != '{') {
                let i1 = response.split('inventory = ')[1].split(';')[0];    // should get everything from { to }, inclusive.
                response = i1;
            }
            let invcache = $.parseJSON(response); //eval('('+response+')');
            let ratchet = invcache[2540]; if (ratchet === undefined) ratch = 0;
            let token = invcache[2317]; if (token === undefined) token = 0;
            let bomb = invcache[2318]; if (bomb === undefined) bomb = 0;
            let html = "You have ";
    ...
    let pyr = document.createElement('div');
...
function at_cave() {
    let dooritem = {"/mus_door1.gif":[37,"a viking helmet"],
    ...
    if (subloc.indexOf("action=door") != -1) {
        ...
        let door = doorpicname.slice(doorpicname.lastIndexOf('/'));
        if (dooritem[door]) {    // item is in our list
            let descid = $('option[value="'+dooritem[door][0]+'"]').attr('descid');
            if (descid) {     // item is in the selection list
                ...
                let failnote = "<center><p><font color='blue'>You need "+dooritem[door][1]+" to proceed.</font></center>";
...
function at_lair1() {
    if (document.location.search == "?action=mirror") {
        $('p:last:contains("Only those comfortable")').append(Util.createLink("[get nekkid]","inv_equip.php?pwd="+pwd+"&action=unequipall"));
        return;
    }
...
function at_lair6() {
    let tabl = document.getElementsByTagName('table');
    img = document.images;
    ...
    if (img.length == 2 && img[1].src.indexOf("gash.gif") != -1) {
        let zif = img[1].parentNode.parentNode;
...
// door and familiar coding shamelessly borrowed from Tard's NS Trainer script v0.8
    if (window.location.search == "" && GetPref("lairspoil") == 1) {
        ...
        window.addEventListener("load",function(e) {
            ...
            let oDiv = document.getElementById("nsTools");
...
// Image courtesy of Picklish's Mining Helper script.
    ...
    if (document.location.search.indexOf("mine=1") != -1) {    // don't display quest data in the knob shaft (mine=2), only the dwarf mine (mine=1).
        let oretype = Character.get("oretype");
        let orenumber = Character.get("orenumber");
        if (oretype != '') {
            // first, process any items we've just received.
            let founditem = $('.item').attr("rel");
            let foundamount = 0;
            if (founditem) {
                founditem = integer(founditem.match(/id=(\d+)/)[1]);
                if (founditem >= 363) {
                    foundamount = Character.get("ore"+founditem);
                    foundamount++;
                    SetCharData("ore"+founditem,foundamount);
                }
            }
            // second, display our quest info.
            let msg = "You need 3 "+oretype+" ores.";
            foundamount = Character.get("ore"+orenumber);
...
// track what each square gave us:
    ...
    $('a[href*="mining.php"]').click(function() {
        let a = $(this);
        let setwhich = integer(a.attr("href").match(/which=(\d+)/)[1]);
    ...
    let which = Character.get("mining_which");
    SetCharData("mining_which", 0);
    if (which != 0 && which !== undefined) {
        let got = $('.item tr td img').attr("src");        // see what we got.
    ...
    $('img[alt*="Open Cavern"]').each(function() {        // replace each "Open Cavern" picture with the image we saved.
        ...
        let xc = integer(alt.match(/(\d)/g)[0]);        // convert from "Open Cavern (1, 4) to the number in the "mine=1?which=xx" link.
        let yc = integer(alt.match(/(\d)/g)[1]);
        let foo = (xc + (yc*8));
        if (foo > 54) return;    // that's the last of the mine-able squares
        let myimage = Character.get("which"+foo);
    ...
    if (select) {    // cards to trade in, while at the cashier.
        ...
        if (qty.length == 2) {
            MakeMaxButton(qty[1], function(event) {
                let selectItem = document.getElementsByTagName("select")[0];
                let box = document.getElementsByName('quantity')[1];
                let quant = ParseSelectQuantity(selectItem, ")");
                box.value = quant;
            });
...
function at_campground() {
    let resultsBar = $('td:first:contains("Results")');
        ...
        && resultsBar.parent().next().text().indexOf('eyepiece of the telescope') != -1)
    {
        resultsBar.parent().next().find('p').each(function(t) {
            ...
            let gateInfo = [
                ...
                ["strange shadow",              ['inkwell',             'inkwell',      'adventure.php?snarfblat=390',                  'haunted library (1)',  '1958']],
...
'" target="mainpane">[' + snarf[3] + ']</a></div>';
        ...
        GM_get( 'api.php?what=inventory&for=MrScript', function(txt) {
            let inventory = $.parseJSON(txt);
            $('b[class=combatitem]').each(function() {
                let itemno = $(this).attr('itemno');
...
function at_basement() {
    ...
    for (var i=0, len=ins.length; i<len; i++) {
        ...
        let phial = 0; let temp = ins[i].value;
        let curphial = Character.get("phial");
        ...
        if (phial > 0 && phial != (curphial+1448)) { // In the biz, we call this this the Phial Phudge Phactor.
            ...
            let phnames = {"1637":"Hot","1638":"Cold","1639":"Spooky","1640":"Stench","1641":"Sleaze"};
            ...
            let cr = document.createElement('center');
            bq.appendChild(cr); cr.appendChild(aa);
        }
    }

    // OMGSPOILERS
    let lvl; let str = ""; let trs = document.getElementsByTagName('tr');
    ...
    let bim = document.images[document.images.length-1];
    let bimg = bim.getAttribute('src');
...
function at_bathole() {
    GM_get( "api.php?what=inventory&for=MrScript", function(rv) {
        let inv = $.parseJSON(rv);
        let sonar = inv['563'];
...
var Krakrox = {
    ...
    let QuestStatus = Character.get("Krakrox");
    if (QuestStatus == null) { QuestStatus = "A"; SetCharData("Krakrox",QuestStatus); }
    let spoilText = Krakrox[cNum][QuestStatus];
...
function spoil_manor1() {
    $('#manor1_pantry > a > img').attr('title','ML: 1-2');
    $('#manor1_kitchen > a > img').attr('title','ML: 3-5;\nboost stench and hot resistance for quest');
    $('#manor1_convervatory > a > img').attr('title','ML: 7-9');
    $('#manor1_library > a > img').attr('title','ML: 51-57;\nfight writing desks for quest');
    $('#manor1_billiards > a > img').attr('title','ML: 20;\nboost NC, Pool skill for quest');
}
...
function spoil_manor2() {
    $('#manor2_bathroom img').attr('title','ML: 88-95\nboost NC, defeat cosmetics wraith for quest');
    $('#manor2_bedroom img').attr('title','ML: 55-60\nfight elegant nightstand, top choice for quest');
    $('#manor2_gallery img').attr('title','ML: 56-60\n boost NC, enter Louvre for quest');
    $('#manor2_ballroom img').attr('title','ML: 65-110');
}
...
function spoil_manor3() {
    ...
    $('#manor3_lab img').attr('title','ML: 80-84');
    $('#manor3_nursery img').attr('title','ML: 76-88');
    $('#manor3_storage img').attr('title','ML: 80-85');
}

function spoil_manor4() {
    $('#manor4_boilerroom img').attr('title','ML: 134-164\nfight monstrous boiler with unstable fulminate equipped\nto get wine bomb for shortcut');
    $('#manor4_chamberwall img').attr('title','equip spectacles to see shortcut');
    $('#manor4_laundryroom img').attr('title','ML: 137-158\nblasting soda from Cabinet of Dr. Limpieza here');
    $('#maonr4_winecellar img').attr('title','ML: 148-168\nChateau de Vinegar from possessed wine rack here');
    $('#manor4_chamberboss img').attr('title','ML: 165\n(Lord Spookyraven)');
}
function spoil_beach() {
    $('img').each(function() {
        let ml = null; let src = this.getAttribute('src');
...
function spoil_cobbsknob() {
    $('area').each(function() {
        let ml = null; let alt = this.getAttribute('alt');
...
function spoil_tavern() {
    $('area').each(function() {
        let ml = null; let src = this.getAttribute('title');
...
function spoil_lair3() {
    let hedge = $('img[src*="hedgemaze.gif"]');
    if (hedge.length>0) {
        hedge.attr('title','ML: 232');
        $('img[src*="castletoptower.gif"]')
            .before(Util.createLink('[hedge puzzle]', 'hedgepuzzle.php', false))
            .before('<br /><br />')
...
function at_topmenu() {
    ...
    if (GetPref('moonslink')== 1) {
        let moons = $('img[src*="moon"]');
        let calendarURL = 'http://noblesse-oblige.org/calendar/';
        let moonlink = document.createElement('a');
...
//end moonslink
    ...
    if (integer(Character.get('level')) < 10) {
//        SetCharData("corner178",0);
//        SetCharData("corner179",0);
//        SetCharData("corner180",0);
//        SetCharData("corner181",0);
...
//        SetCharData("altar4",'');
    ...
    let compactmode = document.getElementsByName('loc').length; // compact mode has a dropdown listbox called 'loc', full mode doesn't.
    if (compactmode > 0) {
        at_compactmenu();
        return;
    }
    let iconmode = document.getElementsByClassName('menuitem').length;
    if (iconmode > 0) {
        at_iconmenu();
        return;
    }
    ...
    if ($('center:first').html().indexOf("javascript:skillsoff();") != -1)
    {
        ...
        let topframeset = top.document.getElementsByTagName && top.document.getElementsByTagName("frameset");
    ...
    let toprow1 = 0, toprow2, toprow3, front;
    let shorttop = 0, haveLair = 0, weBuildHere;
    ...
    $('a').each(function(ind) {
        ...
        // prefs
        let splitinv = GetPref('splitinv');
        let splitquest = GetPref('splitquest');
        let splitmsg = GetPref('splitmsg');
        let logoutconf = GetPref('logout');
        ...
        if (txt == "ship") a.attr("href",Util.getPlaceUrl("bugbearship"));
...
'innerHTML=\' (nope)\'; return false; }">log out</a>');
        ...
        if (txt == "plains") {
            ...
            if (integer(Character.get('level')) > 9)
                a.after(' <a href="' + Util.getPlaceUrl('beanstalk') + '" target="mainpane">stalk</a>');
        ...
        if (txt == "beach") {
            if (integer(Character.get('level')) > 12)
            a.after(' <a href="' + Util.getPlaceUrl("thesea") +'" target="mainpane">sea</a>');
        }
...
//            a.after(' <a href="forestvillage.php" target="mainpane">F-ville</a>');
    ...
    let swordGuy = $('img:first[src*="smallleft"]');
    let swordGuyURL = GetPref('swordguy');
    if (swordGuyURL != '' && swordGuy.length > 0) {
        let guy = document.createElement('a');
        ...
        swordGuy.get(0).addEventListener('contextmenu', function(event) {
            let nuhref = prompt('Where would you like this link to point?',
                GetPref('swordguy'));
            let ln = this.parentNode;
            if (nuhref) {
    ...
    if (shorttop) {
        ...
        let poop = document.createElement('span'); poop.innerHTML = "&nbsp;";
        toprow1.appendChild(poop);
                        AddTopLink(toprow1, 'mainpane', 'multiuse.php', 'multi-use', 1);
        let aPaste =    AddTopLink(toprow1, 'mainpane', 'craft.php?mode=combine', 'combine', 1);
                        AddTopLink(toprow1, 'mainpane', 'sellstuff.php', 'sell', 1);
        let aCook =        AddTopLink(toprow1, 'mainpane', 'craft.php?mode=cook', 'cook', 1);
        let aBooze =    AddTopLink(toprow1, 'mainpane', 'craft.php?mode=cocktail', 'mix', 1);
        let aSmith =    AddTopLink(toprow1, 'mainpane', 'craft.php?mode=smith', 'smith', 1);
        ...
        if (haveLair == 1 && integer(Character.get('level')) == 13)
        ...
        GM_get(Util.getPlaceUrl('forestvillage&action=fv_friar'), function(t) {
            if (t.length>10 && t.indexOf("Back to the Distant Woods") == -1) {
                $('#florist').html('florist')
                    .attr('href',Util.getPlaceUrl('forestvillage&action=fv_friar'));
        ...
        a.addEventListener('click', function(event) {
            let tr1 = document.getElementsByName("toprow1")[0];
            let tr2 = document.getElementsByName("toprow2")[0];
        ...
        for (var j=0; j<10; j++) {
            let zoiks = GetPref('menu2link'+j); let tarjay = 'mainpane';
            if (zoiks != "") {
...
function at_compactmenu() {
    let selectItem, links, oonTD, linkTD;
    let quickSkills = 0, moveqs = 0;
    ...
    for (var i=0, len=links.length; i<len; i++) {
        ...
        if (temp.innerHTML.indexOf("20") != -1) {
            if (moveqs > 0 && quickSkills > 0) {
                ...
                let iframe = document.getElementsByName('skillpane')[0];
                let menuspan = document.getElementsByName('menus')[0];
    ...
    if (GetPref('shortlinks') % 2 > 0) {
        ...
        for (var i=0, len = selectItem.options.length; i<len; i++) {
            if (selectItem.options[i].innerHTML.indexOf("Nearby Plains") != -1) {
                AddTopOption("The Beanstalk", Util.getPlaceUrl("beanstalk"), selectItem, selectItem.options[i+1]);
            ...
            if (selectItem.options[i].innerHTML.indexOf("Desert Beach") != -1) {
                if (integer(Character.get('level')) > 12) {
                    AddTopOption("The Sea",Util.getPlaceUrl("thesea"),selectItem, selectItem.options[i+1]);
                    // len++;
                }
            }
        }
        ...
        AddTopOption("Untinker",Util.getPlaceUrl('forestvillage&action=fv_untinker'), selectItem, 0);
        AddTopOption("Mystic Crackpot", Util.getPlaceUrl("forestvillage&action=fv_mystic"), selectItem, 0);
        ...
        GM_get(Util.getPlaceUrl('forestvillage&action=fv_friar'), function(t) {
            if (t.length>10 && t.indexOf("Back to the Distant Woods") == -1) {
                AddTopOption("Florist",Util.getPlaceUrl('forestvillage&action=fv_friar'),selectItem, 0);
                SetCharData("hasflorist",true);
            } else {
                SetCharData("hasflorist",false);
            }
...
function buildPrefs() {
    ...
    let scriptID = "MrScript";
    let scriptName = "Mr. Script's Choicetastic Optionarium";
    if (!document.querySelector('#scripts')) {
        //scripts tab is not built, do it here
        let scripts = document.querySelector('ul').appendChild(document.createElement('li'));
        scripts.id = 'scripts';
        let a = scripts.appendChild(document.createElement('a'));
    ...
    function createPrefPage(prefSpan) {
        ...
        let choice = MakeOption("Clicking Number Boxes: ", 3, 'autoclear', "Does Zilch", "Clears");
        let select = choice.firstChild.cells[1].firstChild;
...
//        prefSpan.appendChild(MakeOption("Mall Link -> Search", 2, 'malllink', "Off", "On"));
    ...
    function createMenu1(menu1Span) {
        ...
        for (var j=0; j<10; j++) {
            let menutxt = GetPref('menu1link'+j);
        ...
        select.addEventListener('click',function(event) {
            ...
            Defaults(1);
            for (var i=0; i<10; i++) {
                let tag = document.getElementsByName('menu1link'+i+'tag')[0];
                let link = document.getElementsByName('menu1link'+i)[0];
        ...
        choice.addEventListener('click',function(event) {
            event.stopPropagation(); event.preventDefault();
            for (var i=0; i<10; i++) {
                let tag = document.getElementsByName('menu1link'+i+'tag')[0].value;
                let link = document.getElementsByName('menu1link'+i)[0].value;
    ...
    function createMenu2(menu2Span) {
        ...
        for (var j=0; j<10; j++) {
            let menutxt = GetPref('menu2link'+j);
            if (menutxt != undefined) menutxt = menutxt.split(';')[0];
            else menutxt = "";
        ...
        select.addEventListener('click',function(event) {
            ...
            Defaults(2);
            for (var i=0; i<10; i++) {
                let tag = document.getElementsByName('menu2link'+i+'tag')[0];
                let link = document.getElementsByName('menu2link'+i)[0];
        ...
        choice.addEventListener('click',function(event) {
            for (var i=0; i<10; i++) {
                let tag = document.getElementsByName('menu2link'+i+'tag')[0].value;
                let link = document.getElementsByName('menu2link'+i)[0].value;
                if (tag != undefined && link != undefined && tag != "")
    ...
    function createBottomBar(centeredlinks) {
        let ulspan = document.createElement('div');
        let ul4 = document.createElement('a');
        ...
        {    this.innerHTML = 'Working...';
            GM_get( 'store.php?whichstore=m', function(txt)
            {    let nupwd = txt.match(/phash\svalue\=\"([a-z0-9]+)\"/)[1];
    ...
    function buildSettings() {
        ...
        let clicky1 = 'javascript:document.getElementById("scriptpref1").setAttribute("style","");' +
        ...
        let clicky2 = 'javascript:document.getElementById("scriptpref1").setAttribute("style","display:none;");' +
        'javascript:document.getElementById("scriptpref2").setAttribute("style","");' +
        'javascript:document.getElementById("scriptpref3").setAttribute("style","display:none;");';
        let clicky3 = 'javascript:document.getElementById("scriptpref1").setAttribute("style","display:none;");' +
        'javascript:document.getElementById("scriptpref2").setAttribute("style","display:none;");' +
        ...
        let menu1Span = document.createElement('span');
        let menu2Span = document.createElement('span');
        ...
        let centeredlinks = document.createElement('center');
...
function autoclear_added_rows() {
    $('a[href^="javascript"]').each(function()
    {
        ...
        {    let prefAutoclear = GetPref('autoclear');
