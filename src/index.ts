...
function process_effect(effectname, jnode) {
    switch (effectname) {
        case 'Filthworm Larva Stench':
            jnode.after(AppendLink('[drone chamber (1)]', snarfblat(128)));
            break;
        case 'Filthworm Drone Stench':
            jnode.after(AppendLink('[guard chamber (1)]', snarfblat(129)));
            break;
        case 'Filthworm Guard Stench':
            jnode.after(AppendLink('[Queen! (1)]', snarfblat(130)));
            break;
        case 'Stone-Faced':
            jnode.after(AppendLink('[hidden temple (1)]', snarfblat(280)));
            break;
        case 'Down the Rabbit Hole':
            jnode.after(AppendLink('[go down]', to_place('rabbithole')))
                .after(AppendLink('[tea party!]', to_place('rabbithole&action=rabbithole_teaparty')));
            break;
        case 'Knob Goblin Perfume':
...
function process_equip(itemname, jnode) {
    switch (itemname) {
        case 'continuum transfunctioner':
            jnode.after(AppendLink('[8-bit realm (1)]', snarfblat(73)));
            break;
        case 'huge mirror shard':
            jnode.after(AppendLink('[chamber]', 'lair6.php?place=1'));
            break;
        case 'snorkel':
            jnode.after(AppendLink('[map]', inv_use(26)));
            break;
        case 'pool cue':
            jnode.after(AppendLink('[chalk it!]', inv_use(1794)));
            break;
        case "Talisman o' Nam":
            jnode.after(AppendLink('[Dome moD]', to_place('palindome')));
            break;
        case 'worm-riding hooks':
            jnode.after(AppendLink('[drum!]', inv_use(2328)));
            break;
        case 'Mega Gem':
            jnode.after(AppendLink("[Dr. Awkward Office (1)]", to_place('palindome&action=pal_droffice')));
            break;
        case 'dingy planks':
            jnode.after(AppendLink('[boat]', inv_use(146)));
            break;
        case "makeshift SCUBA gear":
            jnode.after(AppendLink('[odor]', 'lair2.php?action=odor'));
            break;
        case 'worthless gew-gaw':
        case 'worthless knick-knack':
        case 'worthless trinket':
            jnode.after(AppendLink('[hermit]', 'hermit.php'));
            break;
        case 'antique machete':
            jnode.after(AppendLink('[hidden city]', 'hiddencity.php'));
            break;
        ...
        case "hilarious comedy prop":
            jnode.after(AppendLink('[visit Mourn]', 'pandamonium.php?action=mourn'));
            break;
    }
}
...
function FindMaxQuantity(item, howMany, deefault, safeLevel) {
    ...
    switch (integer(item)) {
        ...
        case 2370:
            min = 80;
            max = 120;
            break;  // fennel Sooooooda
        case 2378:
            min = 40;
            max = 100;
            break;  // banana spritzer
        case 2437:
            min = 140;
            max = 160;
            break; // New Cloke!
        ...
        case 3697:
            min = 150;
            max = 200;
            break; // high-pressure seltzer bottle
        case 3727:
            min = 50;
            max = 70;
            break;  	// Nardz
        ...
        case 231:
            ...
            max = 5;
            hp = 1;
            break;  // Doc G's Pungent Unguent
        case 232:
            min = 8;
            max = 10;
            hp = 1;
            break;  // Doc G's Ailment Ointment
        case 233:
            min = 13;
            max = 15;
            hp = 1;
            break;  // Doc G's Restorative Balm
        case 234:
            min = 18;
            max = 20;
            hp = 1;
            break;  // Doc G's Homeopathic Elixir
        case 474:
            min = 15;
            max = 20;
            hp = 1;
    ...
    GM_xmlhttpRequest({
        ...
        onerror: function (error) {
            if (typeof (errCallback) == 'function') {
                ...
                GM_log("finalURL=" + error.finalURL);
            }
        },
...
function InvChecker(event) {
    ...
    var item = this.parentNode.previousSibling.firstChild;
    if (item === null) {
...
function ajaxit(dourl) {
    $.ajax({
        type: 'GET', url: dourl + "&ajax=1", cache: false, data: null, global: false,
...
function AddLinks(descId, theItem, formWhere, path) {
    ...
    switch (integer(itemNum)) {
        ...
        case 2334:
        case    2766:
        case  2829:										// MacGuffin, solid gold bowling ball, really dense meat stack
        ...
        case  609:                                                              // S.O.C.K
            addWhere.append(AppendLink('[giant Basement (1)]', snarfblat(322)));
            break;
...
//							'&quantity=1'));
        ...
        case 5785:
        case 5786:
        case 5787:  // smut orc building materials
            addWhere.append(AppendLink('[build!]', to_place('orc_chasm&action=bridge')));
            break;
        case 6693:
        case 6694:                                                  // McClusky file, page 5, binder clip
            addWhere.append(AppendLink('[bind!]', inv_use(6694)));
            break;
        case 7179:
        case 7182:
        case 7184:                                      // First Pizza, Stankara Stone, Shield of Brook
        case 7181:
        case 7183:
        case 7180:
            addWhere.append(AppendLink('[Copperhead Club (1)]', snarfblat(383)));
            break;
        case 7185:
        case 7178:                                                   // copperhead charms
            addWhere.append(AppendLink('[man, o nam]', 'craft.php?mode=combine&' +
                'action=craft&a=7185&b=7178&pwd=' + pwd +
                '&qty=1'));
            break;
        case 7262:
        case 7270:                                                   // I Love Me, 2 Love Me
            doWhat = "oneuse";
...
function SkillUseLimit(skillNum) {
    ...
    if (max != 0) {
        ...
        switch (safeLevel) {
            case 0:
                limit = integer(0.5 + hp / ((min + max) / 2.0));
                break;
            case 1:
                limit = integer(0.5 + hp / (((max * 2) + min) / 3.0));
                break;
            case 2:
                limit = integer(0.5 + hp / max);
                break;
        }
    }
    return limit;
...
function Defaults(revert) {
    ...
    if (revert == 0) {
        for (i = 0; i < basePrefs.length; i++) {
            if (GetPref(basePrefs[i][0]) == undefined) SetPref(basePrefs[i][0], basePrefs[i][1])
        }
        for (i = 0; i < menu1.length; i++) {
            if (GetPref("menu1link" + i) == undefined) SetPref("menu1link" + i, menu1[i])
        }
        for (i = 0; i < menu2.length; i++) {
            if (GetPref("menu2link" + i) == undefined) SetPref("menu2link" + i, menu2[i])
        }
    } else if (revert == 1) {
        for (i = 0; i < menu1.length; i++) {
            SetPref("menu1link" + i, menu1[i])
        }
    } else if (revert == 2) {
        for (i = 0; i < menu2.length; i++) {
            SetPref("menu2link" + i, menu2[i])
        }
    }
}
...
function InlineItemDescriptions() {
    ...
    $("a[onclick*='descitem(']").addClass("hand");
...
// code for NS Lair spoilers borrowed shamelessly from Tard's NS Trainer v0.8
    ...
    $('option[value="2947"]').each(function () {
        ...
        if (insultsList == undefined) insultsList = "0;0;0;0;0;0;0;0";
...
function at_cove() {
    ...
    $('#H_insultreset').click(function () {
        SetCharData("insults", "0;0;0;0;0;0;0;0");
        window.location = "cove.php";
    });
}
...
function at_cobbsknob() {
    $('p').each(function () {
        ...
        if ((txt.indexOf("I'll give it a shot") != -1) ||
            (txt.indexOf("I'll get right on it") != -1)) {
            $(this).append(AppendLink('[lab (1)]', snarfblat(50)));
        }
        if ((txt.indexOf("them on Menagerie level 1") != -1) ||
            (txt.indexOf("First, I'm going to need") != -1)) {
            $(this).append(AppendLink('[menagerie-1 (1)]', snarfblat(51)));
        } else if ((txt.indexOf("Okay, the next thing") != -1) ||
            (txt.indexOf("flartble") != -1)) {
            $(this).append(AppendLink('[menagerie-2 (1)]', snarfblat(52)));
        } else if ((txt.indexOf("This is the last thing I need") != -1) ||
            (txt.indexOf("be right back") != -1)) {
            $(this).append(AppendLink('[menagerie-3 (1)]', snarfblat(53)));
        }
    });
    ...
    switch (subloc) {
        case "?place=paco":
            if (tdtext.indexOf("White Citadel") != -1) {
                ...
                td.append(AppendLink('[Road to WC (1)]', snarfblat(413)));
...
//	var p=document.getElementsByTagName('p');
    if (p.length) {
        ...
        if (p0text.indexOf("actually a book.") != -1) {	// The Oracle
            ...
            p0.appendChild(AppendLink('[to the basement]', to_place('manor4')));
        } else {
...
function at_bounty() {
    ...
    $('img[width=30][class!=hand]').each(function (n, el) {
        var $img = $(this);
        if ($img.attr("title") === "filthy lucre") return true;
...
function at_craft() {
    ...
    if (tbltext.indexOf("more advanced cooking appliance") != -1) {
        itemNeeded = 157;
        desc = "Buy & Install a Dramatic Range";
        store = 'm';
    } else if (tbltext.indexOf("cocktail set is not advanced") != -1) {
        itemNeeded = 236;
        desc = "Buy & Install a Queue du Coq kit";
        store = 'm';
    } else if (tbltext.indexOf("haven't got a hammer") != -1) {
        itemNeeded = 338;
        desc = "Buy a tenderizing hammer";
        store = 's';
    }
    ...
    if (GetPref('shortlinks') > 1) {
        ...
        if (txt.indexOf("Hermit Permit required,") != -1) {			// no permit
            ...
            GM_get(server + 'api.php?what=inventory&for=MrScript', function (response) {		// Check gum and permit counts.
                var invcache = $.parseJSON(response);
                var gum = invcache[23];
                if (gum === undefined) gum = 0;
                gum = integer(gum);
                var hpermit = invcache[42];
                if (hpermit === undefined) hpermit = 0;
    ...
    if (GetPref('shortlinks') > 1) {
        $('p').each(function () {
            ...
            if (txt.indexOf("invaded!") != -1 || txt.indexOf("pirates") != -1) {
                ...
                p.append(AppendLink('[forest (1)]', snarfblat(405)));
...
// Here we put all the quests that have variable text in their labels, like %Playername.
            if (txt.indexOf("White Citadel") != -1)
                ...
                if (subtext.indexOf("find the Black Market") != -1)
                    b.append(AppendLink("[black forest (1)]", snarfblat(405)));
...
function CompressThrallAndFamiliar(compressLevel) {
    ...
    if (compressLevel == 2) {   //show weight and XP only
        ...
        $fp2.next('font')
            ...
            .appendTo($fp1);
        $fp2.hide();
        if (reanimator) $fp1.append(AppendLink('chat!', chatlink));
    } else {                    //show nothing
        $fp2.parent().hide();
    }
...
//	SetData("charname",bText[0].textContent);
    ...
    if (compactMode) {
        ...
        SetCharData("currentHP", curHP);
        SetCharData("maxHP", maxHP);
        SetCharData("currentMP", curMP);
        SetCharData("maxMP", maxMP);
    } else { // Full Mode
        ...
        if (GetPref('backup')) {
            ...
            img.addEventListener('contextmenu', function (event) {
                GM_get(server + 'inv_equip.php?action=customoutfit&which=2&outfitname=' +
                    ...
                    var msg;
                    if (response.indexOf("custom outfits") == -1) msg = "Outfit Backed Up";
            ...
            if (noDisable > 0 && sel.childNodes[i].getAttribute('disabled') != null) {
                switch (integer(temp)) {
                    ...
                    case 3006:
                        break;
                    default:
                        sel.childNodes[i].removeAttribute('disabled');
                        break;
                }
            }
...
function at_clan_viplounge() {
    if (document.location.search == "?action=klaw" && GetPref('klaw') == 1) {
        ...
        $('option').each(function () {
            ...
            switch (this.value) {
                case "1":
                    addme = ' (3-4 shards of double ice)';
                    break;
                case "2":
                    addme = ' (50 turns of +5% moxie)';
                    break;
                case "3":
                    addme = ' (50 turns of +5% myst)';
                    break;
                case "4":
                    addme = ' (50 turns of +5% muscle)';
                    break;
                case "5":
                    addme = ' (+1000 MP, chance of a recipe)';
                    break;
                default:
                    addme = ' (???)';
                    break;
...
function at_pyramid() {
    ...
    checkInv.addEventListener('click', function (evt) {
        ...
        GM_get(server + 'api.php?what=inventory&for=MrScript', function (response) {
            ...
            var invcache = $.parseJSON(response); //eval('('+response+')');
            var ratchet = invcache[2540];
            if (ratchet === undefined) ratch = 0;
            var token = invcache[2317];
            if (token === undefined) token = 0;
            var bomb = invcache[2318];
...
function at_cave() {
    var dooritem = {
        "/mus_door1.gif": [37, "a viking helmet"],
...
// door and familiar coding shamelessly borrowed from Tard's NS Trainer script v0.8
    if (window.location.search == "" && GetPref("lairspoil") == 1) {
        ...
        window.addEventListener("load", function (e) {
            ...
            newDiv.id = "nsToolsHead";
            newDiv.innerHTML = "Naughty Sorceress Tools:";
            ...
            if (bodyHTML.indexOf("goat.gif") != -1) {
                ...
                famId = 5;
                famName = "Sabre-Toothed Lime";
            } else if (bodyHTML.indexOf("lime.gif") != -1) {
                famId = 3;
                famName = "Levitating Potato";
...
function at_basement() {
    ...
    for (var i = 0, len = ins.length; i < len; i++) {
        ...
        if (phial > 0 && phial != (curphial + 1448)) { // In the biz, we call this this the Phial Phudge Phactor.
            ...
            aa.innerHTML = "Use " + phnames[phial] + " Phial";
...
function spoil_Krakrox(cNum) {
    var Krakrox = {
        ...
        372: {
            D: "Climb down well (fight giant octopus)",
            E: "Take grappling hook"
        },
        373: {
            K: "Place both blocks and pull lever, then go south",
            L: "Go South",
        ...
        374: {
            B: "Climb Tower (fight giant bird-creature)",
            C: "Climb Tower (get memory of half a stone circle)",
        },
        375: {
            F: "Go Upstairs (get memory of an iron key)",
            G: "Go Downstairs",
            H: "Go Downstairs",
        },
        376: {
        ...
        384: {
            L: "Open Chest, then smash it (get memory of a stone half-circle)"
        },
        385: {
            M: "Go North"
        },
        386: {
            M: "Ponder the Weights"
        }
    }
    var QuestStatus = GetCharData("Krakrox");
    if (QuestStatus == null) {
        QuestStatus = "A";
        SetCharData("Krakrox", QuestStatus);
    }
...
//			a.after(' <a href="forestvillage.php" target="mainpane">F-ville</a>');
        ...
        if (txt == "messages" && splitmsg > 0) {
            switch (splitmsg) {
                case 2:
                    a.attr('href', 'messages.php?box=Outbox');
                    break;
                case 3:
                    a.attr('href', 'messages.php?box=Saved');
                    break;
                case 4:
                    a.attr('href', 'messages.php?box=PvP');
                    break;
                default:
                    a.attr('href', 'sendmessage.php');
            }
...
//		prefSpan.appendChild(MakeOption("Mall Link -> Search", 2, 'malllink', "Off", "On"));
    ...
    function createMenu1(menu1Span) {
        ...
        select.innerHTML = 'Restore Defaults';
        select.href = '#';
